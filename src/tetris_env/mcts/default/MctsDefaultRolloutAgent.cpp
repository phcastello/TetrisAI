...
SearchResult runSearch(const TetrisEnv& env,
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        // Rollout: escolhe acoes aleatorias uniformemente.
        ...
        if (!rolloutNode.terminal && depth < params.maxDepth) {
            while (!sim.isGameOver() && depth < params.maxDepth) {
                ...
                Action a = randomAction(actions, rng);
...
MctsDefaultRolloutAgent::MctsDefaultRolloutAgent(const MctsParams& params) : params_(params) {
